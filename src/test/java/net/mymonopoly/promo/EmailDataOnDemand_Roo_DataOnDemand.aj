...
privileged aspect EmailDataOnDemand_Roo_DataOnDemand {
    ...
    public void EmailDataOnDemand.setSubscribeDate(Email obj, int index) {
        Date subscribeDate = new GregorianCalendar(Calendar.getInstance().get(Calendar.YEAR), Calendar.getInstance().get(Calendar.MONTH), Calendar.getInstance().get(Calendar.DAY_OF_MONTH), Calendar.getInstance().get(Calendar.HOUR_OF_DAY), Calendar.getInstance().get(Calendar.MINUTE), Calendar.getInstance().get(Calendar.SECOND) + new Double(Math.random() * 1000).intValue()).getTime();
        obj.setSubscribeDate(subscribeDate);
    }
